...
可以使用常规 Python 数字运算符将单位和数量组合在一起。 
例如::
...
单位支持分数幂, 通过复杂的操作保持其精度. 
因此, 建议使用 `fractions.Fraction` 对象.
例如::
...
.. note::
    ...
    与分母小于10的分数有效相同的浮点幂将隐式转换为引擎盖下的“~resractions.Fraction”对象。 
    因此以下是等同的::
...
using the `~astropy.units.def_unit` function.  例如::
